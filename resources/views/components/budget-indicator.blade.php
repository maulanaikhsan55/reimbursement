...
>
    <div class="budget-header">
        <div class="budget-title">
            ...
            <span>Anggaran {{ $departmentName ?? 'Departemen' }}</span>
        </div>
    ...
    <div class="progress-wrapper">
        <div id="budget-progress-bar" class="progress-bar progress-{{ $status['status'] }}" style="width: {{ min($status['percentage'], 100) }}%"></div>
    </div>

    <div class="budget-info-footer">
        <div class="usage-info">
            <span class="label">Terpakai:</span>
            <span class="value" id="budget-usage-value">Rp {{ number_format($status['usage'] + $status['current'], 0, ',', '.') }}</span>
        </div>
    ...
    <div id="budget-warning-box" class="budget-warning {{ $status['is_over'] ? '' : 'd-none' }}">
