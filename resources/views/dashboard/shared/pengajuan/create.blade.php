...
@php
    $routePrefix = $routePrefix ?? (request()->routeIs('atasan.*') ? 'atasan' : 'pegawai');
@endphp
    ...
    <div class="dashboard-content">
        <section class="modern-section">
            <div class="section-header">
                ...
                <a href="{{ route($routePrefix.'.pengajuan.index') }}" class="link-back">
            ...
            <div class="create-layout-grid">
                ...
                <form action="{{ route($routePrefix.'.pengajuan.store') }}" method="POST" enctype="multipart/form-data" class="form-pengajuan create-main-form">
                @csrf
                ...
                <input type="hidden" id="jenis_transaksi" name="jenis_transaksi" value="other">

                <div class="form-row">
                    <div class="form-group" style="grid-column: 1 / -1;">
                        <label for="judul" class="form-label" style="font-size: 0.9rem;">Judul / Deskripsi Singkat <span class="required">*</span></label>
                        <input
                            type="text"
                            class="form-input @error('judul') error @enderror"
                            id="judul"
                            name="judul"
                            value="{{ old('judul', isset($duplicateFrom) ? ($duplicateFrom->judul ?: ($duplicateFrom->deskripsi_singkat ?? '')) : '') }}"
                            placeholder="Contoh: Reimburse ATK tim sales minggu ini"
                            required
                ...
                <div class="form-row">
                    <div class="form-group">
                        <label for="nama_vendor" class="form-label" style="font-size: 0.9rem;">Vendor / Penerima <span class="required">*</span></label>
                        <input type="text" class="form-input @error('nama_vendor') error @enderror" id="nama_vendor" name="nama_vendor" value="{{ old('nama_vendor', isset($duplicateFrom) ? $duplicateFrom->nama_vendor : '') }}" placeholder="Contoh: PT ABC, Toko XYZ, Nama Penerima" required style="font-size: 0.9rem;">
                        <small style="color: #64748b; font-size: 0.75rem; display: flex; align-items: center; gap: 0.25rem; margin-top: 0.2rem;">
                            <x-icon name="alert-circle" class="w-3 h-3" />
                            <span>Isi nama vendor/merchant atau penerima transfer sesuai invoice</span>
                        </small>
                        @error('nama_vendor')
                            <span class="error-message">{{ $message }}</span>
                        @enderror
                ...
                <div class="form-group">
                    <label for="deskripsi" class="form-label" style="font-size: 0.9rem;">Catatan Detail / Deskripsi <span class="required">*</span></label>
                    <textarea class="form-input @error('deskripsi') error @enderror" id="deskripsi" name="deskripsi" rows="3" placeholder="Jelaskan detail kebutuhan/aktivitas yang dibiayai" required style="font-size: 0.9rem; min-height: 96px;">{{ old('deskripsi', isset($duplicateFrom) ? $duplicateFrom->deskripsi : '') }}</textarea>
                    <small style="color: #64748b; font-size: 0.75rem; display: flex; align-items: center; gap: 0.25rem; margin-top: 0.2rem;">
                        <x-icon name="alert-circle" class="w-3 h-3" />
                        <span>Contoh: "Pembelian ATK untuk operasional tim minggu ini"</span>
                    </small>
                    @error('deskripsi')
                        <span class="error-message">{{ $message }}</span>
                    @enderror
                ...
                <div class="form-group">
                    ...
                    <div class="file-upload @error('file_bukti') error @enderror" id="fileUploadZone" style="padding: 2rem 1.5rem;">
                        ...
                        <div class="file-upload-text" style="gap: 0.5rem;" id="uploadContent">
                            ...
                            <div class="upload-doc-tags">
                                <span class="upload-doc-tag">
                                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                            ...
                            <span style="font-size: 0.75rem;">Format: JPG, PNG, WebP, PDF | Maks 5MB</span>
                            <span style="font-size: 0.72rem; color: #64748b;">Pastikan dokumen invoice/struk terlihat jelas (gambar atau PDF).</span>
                        </div>
                    </div>
                    ...
                    <div id="ultra-smart-summary" class="ultra-smart-container" style="display: none;">
                        ...
                        <div class="smart-grid">
                            ...
                            <div class="smart-side-stack">
                                ...
                                <div class="smart-card">
                                    <p class="smart-card-title">Insight Transaksi</p>
                                    <div id="smart-transaction-time" class="smart-score">Waktu: -</div>
                                    <div id="smart-anomaly-flag" class="smart-note">Analisis anomali menunggu hasil OCR.</div>
                                </div>
                ...
                <div class="form-actions" style="margin-top: 2.5rem; padding-top: 1.5rem; border-top: 1px solid #f1f5f9; display: flex; justify-content: flex-end; align-items: center; gap: 1rem;">
                    <a href="{{ route($routePrefix.'.pengajuan.index') }}" class="btn-modern btn-modern-secondary" style="min-width: 120px;">
                        Batal
                    </a>
...
<style>
    ...
    .form-input.client-field-invalid {
    ...
    .file-upload.client-field-invalid {
        border-color: #dc2626 !important;
        background: #fff1f2;
    }
...
<script>
    ...
    const OCR_ROUTES = {
        process: "{{ route($routePrefix.'.validasi-ai.process-file') }}",
        validate: "{{ route($routePrefix.'.validasi-ai.validate-input') }}"
    };
    ...
    (function () {
        ...
        const parseNominalNumber = (value) => {
            const numeric = Number(String(value ?? '').replace(/[^\d]/g, ''));
            return Number.isFinite(numeric) ? numeric : 0;
        };

        const formatNominalInput = (value) => {
            const num = parseNominalNumber(value);
        ...
        const initOcrCreateForm = () => {
            ...
            const deskripsiDetailInput = document.getElementById('deskripsi');
            ...
            const budgetContainer = document.getElementById('budget-indicator-container');
            const budgetUsageValue = document.getElementById('budget-usage-value');
            ...
            const budgetWarningBox = document.getElementById('budget-warning-box');
            ...
            const smartTransactionTime = document.getElementById('smart-transaction-time');
            const smartAnomalyFlag = document.getElementById('smart-anomaly-flag');
            const kategoriDescription = document.getElementById('kategori-description');
            const kategoriDescriptionText = document.getElementById('description-text');
            const recommendedCoaSection = document.getElementById('recommendedCoaSection');
            const recommendedCoaCode = document.getElementById('recommendedCoaCode');
            const recommendedCoaName = document.getElementById('recommendedCoaName');
            ...
            let shouldShowRequiredErrors = false;
            const requiredBeforeUpload = [
                { el: dateInput, label: 'Tanggal transaksi' },
                { el: kategoriInput, label: 'Kategori biaya' },
                { el: judulInput, label: 'Judul / deskripsi singkat' },
                { el: vendorInput, label: 'Vendor / penerima' },
                { el: nominalInput, label: 'Nominal' },
                { el: deskripsiDetailInput, label: 'Catatan detail / deskripsi' }
            ];
            const allowedFileExtensions = ['jpg', 'jpeg', 'png', 'webp', 'pdf'];
            const maxFileSizeBytes = 5 * 1024 * 1024;
            const uploadIdleMarkup = `
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="width: 28px; height: 28px;">
                    <path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"></path>
                    <polyline points="17 8 12 3 7 8"></polyline>
                    <line x1="12" y1="3" x2="12" y2="15"></line>
            ...
            const budgetUsageBaseRaw = Number(budgetContainer?.dataset?.budgetUsageBase ?? NaN);
            const budgetLimitRaw = Number(budgetContainer?.dataset?.budgetLimit ?? NaN);

            const budgetUsageBase = Number.isFinite(budgetUsageBaseRaw)
                ? budgetUsageBaseRaw
                : parseNominalNumber(budgetUsageValue?.textContent || '');
            const budgetLimit = Number.isFinite(budgetLimitRaw)
                ? budgetLimitRaw
                : parseNominalNumber(budgetLimitValue?.textContent || '');
            ...
            const updateBudgetIndicatorFromNominal = () => {
                if (!budgetContainer || !budgetUsageValue || !budgetPercentageText || !budgetProgressBar || budgetLimit <= 0) {
                    return;
                }

                const nominalNow = parseNominalNumber(nominalInput?.value || '');
                const projectedUsage = budgetUsageBase + nominalNow;
                const percentage = (projectedUsage / budgetLimit) * 100;

                budgetUsageValue.textContent = formatRupiah(projectedUsage);
                budgetPercentageText.textContent = formatPercent(percentage);
                budgetProgressBar.style.width = `${Math.min(Math.max(percentage, 0), 100)}%`;

                budgetProgressBar.classList.remove('progress-success', 'progress-warning', 'progress-danger');
                if (percentage > 100) {
                    budgetProgressBar.classList.add('progress-danger');
                } else if (percentage > 80) {
                    budgetProgressBar.classList.add('progress-warning');
                } else {
                    budgetProgressBar.classList.add('progress-success');
                }

                if (budgetWarningBox) {
                    budgetWarningBox.classList.toggle('d-none', percentage <= 100);
                }
            };

            const setNominalValue = (value) => {
                if (!nominalInput) return;
                nominalInput.value = formatNominalInput(value);
                updateBudgetIndicatorFromNominal();
            };

            const refreshKategoriContext = () => {
                if (!kategoriInput) return;

                const selectedOption = kategoriInput.options[kategoriInput.selectedIndex];
                const hasSelection = !!(kategoriInput.value && selectedOption);

                if (!hasSelection) {
                    if (kategoriDescription) kategoriDescription.style.display = 'none';
                    if (recommendedCoaSection) recommendedCoaSection.style.display = 'none';
                    if (recommendedCoaCode) recommendedCoaCode.textContent = '-';
                    if (recommendedCoaName) recommendedCoaName.textContent = '-';
                    return;
                }

                const description = (selectedOption.getAttribute('data-description') || '').trim();
                if (kategoriDescription && kategoriDescriptionText) {
                    if (description !== '') {
                        kategoriDescriptionText.textContent = description;
                        kategoriDescription.style.display = 'block';
                    } else {
                        kategoriDescription.style.display = 'none';
                    }
                }

                const coaCode = (selectedOption.getAttribute('data-coa-code') || '').trim();
                const coaName = (selectedOption.getAttribute('data-coa-name') || '').trim();

                if (!recommendedCoaSection || !recommendedCoaCode || !recommendedCoaName) {
                    return;
                }

                if (coaCode !== '' && coaName !== '') {
                    recommendedCoaCode.textContent = coaCode;
                    recommendedCoaName.textContent = coaName;
                    recommendedCoaSection.style.display = 'block';
                } else {
                    recommendedCoaCode.textContent = '-';
                    recommendedCoaName.textContent = 'Default COA belum diatur';
                    recommendedCoaSection.style.display = 'block';
                }
            };

            const applyNominalInputFormatting = () => {
                if (!nominalInput) return;
                nominalInput.value = formatNominalInput(nominalInput.value);
                updateBudgetIndicatorFromNominal();
            };

            const debounceValidateInput = (delay = 220, showMissingErrors = false) => {
                clearTimeout(validateTimer);
                validateTimer = setTimeout(() => {
                    void validateInput({ showMissingErrors });
                }, delay);
            };

            const getFieldErrorNode = (el, createIfMissing = false) => {
                if (!el || !el.id) return null;
                const group = el.closest('.form-group');
                if (!group) return null;

                let node = group.querySelector(`[data-client-error-for="${el.id}"]`);
                if (!node && createIfMissing) {
                    node = document.createElement('span');
                    node.className = 'client-field-error';
                    node.setAttribute('data-client-error-for', el.id);
            ...
            const setFieldError = (el, message) => {
                if (!el) return;
                el.classList.add('client-field-invalid');
                const node = getFieldErrorNode(el, true);
                if (!node) return;
            ...
            const clearFieldError = (el) => {
                if (!el) return;
                el.classList.remove('client-field-invalid');
                const node = getFieldErrorNode(el, false);
                if (!node) return;
                node.textContent = '';
            ...
            const clearRequiredFieldErrors = () => {
                requiredBeforeUpload.forEach(({ el }) => clearFieldError(el));
            };

            const renderRequiredFieldErrors = (missingFields) => {
                clearRequiredFieldErrors();
                missingFields.forEach(({ el, label }) => {
                    setFieldError(el, `${label} wajib diisi.`);
                });
            };

            const clearFileUploadError = () => {
                fileUploadZone?.classList.remove('client-field-invalid');
                clearFieldError(fileInput);
            };

            const setFileUploadError = (message) => {
                fileUploadZone?.classList.add('client-field-invalid');
                setFieldError(fileInput, message);
            };
            ...
            const getMissingRequiredFields = () => {
                return requiredBeforeUpload.filter(({ el }) => !isFieldFilled(el));
            };
            ...
            const setSubmitState = () => {
                if (!submitBtn) return;
                const hasRequired = hasRequiredFields();
                const hasFile = !!(fileInput && fileInput.files && fileInput.files.length);
                submitBtn.disabled = !(hasRequired && hasFile && !isDuplicate && canSubmitByValidation);
            };
            ...
            bindCopy(ocrNominal, (val) => {
                if (val && val !== '-') setNominalValue(val);
                debounceValidateInput(0);
            });
            ...
            const extractTransactionTimeFromOcr = (data) => {
                const direct = String(data?.time || '').trim();
                if (/^([01]?\d|2[0-3])[:.]([0-5]\d)$/.test(direct)) {
                    return direct.replace('.', ':');
                }

                const rawText = String(data?.raw_text || '');
                const match = rawText.match(/(?:^|\s)([01]?\d|2[0-3])[:.]([0-5]\d)(?::([0-5]\d))?(?:\s*(wib|wita|wit))?/i);
            ...
            const buildDetailFromRawText = (data) => {
                ...
                const stopwords = [
                    'transaksi berhasil', 'total transaksi', 'total', 'nominal', 'biaya admin', 'admin',
                    'ppn', 'pajak', 'no. ref', 'ref', 'tanggal', 'waktu', 'sumber dana', 'tujuan',
                    'informasi', 'npwp', 'bank', 'keterangan', 'catatan', 'pembayaran', 'invoice'
                ];
                ...
                for (const line of lines) {
                    ...
                    const cleaned = line.replace(/(?:rp|idr)?\s*[0-9][0-9.,\s]{1,20}/gi, '').trim();
                    if (cleaned.length >= 4) {
                ...
                const vendorText = vendor !== '' ? vendor : 'Vendor tidak terdeteksi';
                const nominalText = nominal > 0 ? formatRupiah(nominal) : 'Nominal tidak terdeteksi';
                const tanggalText = tanggal !== '' ? tanggal : 'Tanggal tidak terdeteksi';
                return `${vendorText} | ${nominalText} | ${tanggalText}`;
            };

            const renderSmartInsights = (data) => {
                if (!smartWrap) return;

                const hasMeaningfulInsight = !!(
                    (Array.isArray(data?.items) && data.items.length > 0) ||
                    (typeof data?.detail_transaksi === 'string' && data.detail_transaksi.trim() !== '') ||
                    (typeof data?.sanity_check_notes === 'string' && data.sanity_check_notes.trim() !== '') ||
                    Number(data?.fraud_risk_score || 0) > 0 ||
                    (data?.llm_anomaly_analysis && typeof data.llm_anomaly_analysis === 'object') ||
                    (typeof data?.raw_text === 'string' && data.raw_text.trim() !== '') ||
                    (typeof data?.vendor === 'string' && data.vendor.trim() !== '') ||
                    Number(data?.nominal || 0) > 0
                );

                if (!hasMeaningfulInsight) {
                    smartWrap.style.display = 'none';
                    return;
                }
                ...
                const itemNames = Array.isArray(data?.items)
                    ? data.items.map((item) => typeof item === 'string' ? item : (item?.name || '')).filter(Boolean)
                    : [];
                const detailTransaksi = String(data?.detail_transaksi || '').trim();
                const detailFallback = buildDetailFromRawText(data);
                const finalDetailText = itemNames.length > 0
                    ? itemNames.join(', ')
                    : (detailTransaksi !== '' ? detailTransaksi : (detailFallback || 'Data item tidak tersedia'));
                if (smartItems) {
                    smartItems.textContent = finalDetailText;
                }

                const llmAnomaly = data?.llm_anomaly_analysis || {};
                const llmRiskScore = Number(llmAnomaly?.risk_score || data?.fraud_risk_score || 0);
                const llmRiskLevel = String(llmAnomaly?.risk_level || (llmRiskScore >= 70 ? 'high' : (llmRiskScore >= 40 ? 'medium' : 'low'))).toUpperCase();
                const llmRecommendation = String(llmAnomaly?.approval_recommendation || 'approve').toUpperCase();
                const llmSummary = llmAnomaly?.summary || data?.sanity_check_notes || 'Tidak ada anomali signifikan terdeteksi';
                const llmFlags = Array.isArray(llmAnomaly?.red_flags) ? llmAnomaly.red_flags.filter(Boolean).slice(0, 3) : [];

                if (smartFraudScore) smartFraudScore.textContent = `${llmRiskScore}% (${llmRiskLevel})`;
                if (smartFraudNote) {
                    const flagText = llmFlags.length > 0 ? ` | Red flags: ${llmFlags.join(', ')}` : '';
                    smartFraudNote.textContent = `[${llmRecommendation}] ${llmSummary}${flagText}`;
                }

                if (smartConfidenceScore) {
                    smartConfidenceScore.textContent = data?.confidence_score ? `${data.confidence_score}%` : '-';
                }
                if (smartConfidenceReason) {
                    smartConfidenceReason.textContent = data?.confidence_reason || 'Analisis OCR + AI selesai';
                }
                if (smartCategory) {
                    smartCategory.textContent = data?.suggested_category || data?.predicted_category || data?.kategori || 'Umum';
                }

                const txTime = extractTransactionTimeFromOcr(data);
                if (smartTransactionTime) {
                    smartTransactionTime.textContent = txTime ? `Waktu: ${txTime} WIB` : 'Waktu: tidak terdeteksi';
                }
                if (smartAnomalyFlag) {
                    if (!txTime) {
                        smartAnomalyFlag.textContent = `Ringkasan: ${llmSummary}`;
                    } else {
                        const hour = parseInt(txTime.split(':')[0], 10);
                        const outsideWorkHours = Number.isFinite(hour) && (hour < 8 || hour >= 18);
                        const workHourText = outsideWorkHours
                            ? `Transaksi di luar jam kerja standar (${txTime}).`
                            : `Transaksi pada jam kerja standar (${txTime}).`;
                        smartAnomalyFlag.textContent = `${workHourText} Risiko AI: ${llmRiskScore}% (${llmRiskLevel}).`;
                    }
                }
            };
            ...
            const renderValidation = (result) => {
                ...
                if (statusVendor) {
                    const vendorPct = Number(matches.vendor?.match_percentage ?? 0);
                    const pctText = Number.isFinite(vendorPct) ? `${vendorPct.toFixed(1)}%` : '-';
                    statusVendor.innerHTML = `${statusBadge(matches.vendor?.status)} <span style="font-size:0.72rem;color:#64748b;">(${pctText})</span>`;
                }
                ...
                const anomaly = matches.anomali || {};
                const llmAnomaly = matches.llm_anomaly || {};
                const riskScore = Number(anomaly?.risk_score || 0);
                const riskLevel = String(anomaly?.risk_level || 'low').toUpperCase();
                const llmRiskScore = Number(llmAnomaly?.risk_score || 0);
                const llmRiskLevel = String(llmAnomaly?.risk_level || 'low').toUpperCase();
                const llmDecision = String(llmAnomaly?.recommendation || 'approve').toUpperCase();
                const riskInfo = `Risiko anomali ${riskScore}% (${riskLevel}) | Risiko LLM ${llmRiskScore}% (${llmRiskLevel}) [${llmDecision}]`;

                if (issues.length > 0) {
                    const top = issues[0];
                    setSummary(top.type || 'warning', `${top.title}: ${top.message} | ${riskInfo}`);
                } else {
                    if (llmDecision === 'REJECT') {
                        setSummary('warning', `AI mendeteksi risiko tinggi (rekomendasi review manual). ${riskInfo}. ${llmAnomaly?.decision_reason || llmAnomaly?.summary || ''}`);
                    } else if (riskScore >= 40 || llmDecision === 'REVIEW') {
                        setSummary('warning', `Data valid, namun perlu review: ${riskInfo}. ${llmAnomaly?.decision_reason || anomaly?.reason || ''}`);
                    } else {
                        setSummary('success', `Data valid. Pengajuan siap dikirim. ${riskInfo}.`);
                    }
                }

            };

            const validateInput = async ({ showMissingErrors = false } = {}) => {
                const requestId = ++validateRequestId;
                const missingFields = getMissingRequiredFields();
                if (missingFields.length > 0) {
                    canSubmitByValidation = false;
                    if (showMissingErrors || shouldShowRequiredErrors) {
                        renderRequiredFieldErrors(missingFields);
                    } else {
                        clearRequiredFieldErrors();
                    }
                    setSubmitState();
                ...
                if (!currentOcrData) {
                    canSubmitByValidation = hasRequiredFields();
                    setSubmitState();
                    return;
                }
                try {
            ...
            const processOcr = async () => {
                ...
                if (uploadContent) uploadContent.innerHTML = '<p style="margin:0;font-size:.9rem;color:#4f46e5;">Memproses OCR + AI...</p>';
                ...
                try {
                    ...
                    if (!response.ok || result.success === false) {
                        ...
                        validateRequestId++;
                        canSubmitByValidation = false;
                        setSummary('error', result.message || 'Proses OCR + AI gagal diproses.');
                        setSubmitState();
                        notify('error', 'OCR + AI Gagal', result.message || 'Proses OCR + AI gagal.');
                        return;
                    }
                    ...
                    if (multiOptions) multiOptions.innerHTML = '';

                    if (hasRequiredFields()) {
                        await validateInput();
                        renderSmartInsights(currentOcrData || {});
                    } else {
                        canSubmitByValidation = false;
                        setSummary('info', 'OCR + AI selesai. Lengkapi field wajib untuk menjalankan validasi kesesuaian data.');
                        setSubmitState();
                    }
                    notify('success', 'OCR + AI Selesai', result.message || 'Data struk berhasil dibaca dan dianalisis AI.');
                } catch (e) {
                    currentOcrData = null;
                    validateRequestId++;
                    canSubmitByValidation = false;
                    setSummary('error', 'Terjadi gangguan jaringan saat proses OCR + AI.');
                    setSubmitState();
                } finally {
                    if (uploadContent) {
                        uploadContent.innerHTML = uploadIdleMarkup;
                    }
                }
            };
            ...
            fileInput?.addEventListener('change', () => {
                ...
                if (missingFields.length > 0) {
                    ...
                    canSubmitByValidation = false;
                    shouldShowRequiredErrors = true;
                    renderRequiredFieldErrors(missingFields);
                    if (validationResults) validationResults.style.display = 'none';
                    if (smartWrap) smartWrap.style.display = 'none';
                    setSubmitState();
                    setSummary('warning', `Lengkapi field wajib sebelum upload: ${missingFields.map((field) => field.label).join(', ')}.`);
                    notify('warning', 'Lengkapi Form Dulu', 'Isi semua field wajib sebelum upload bukti transaksi.');
                ...
                if (!allowedFileExtensions.includes(extension)) {
                    ...
                    if (filePreview) filePreview.style.display = 'none';
                    canSubmitByValidation = false;
                    setFileUploadError('Upload hanya mendukung gambar (JPG, PNG, WebP) atau PDF.');
                    setSummary('error', 'Format file tidak didukung. Gunakan JPG, PNG, WebP, atau PDF.');
                    setSubmitState();
                    notify('error', 'Format Tidak Didukung', 'File bukti harus berupa gambar atau PDF.');
                    return;
                }
                ...
                if (file.size > maxFileSizeBytes) {
                    ...
                    if (filePreview) filePreview.style.display = 'none';
                    canSubmitByValidation = false;
                    setFileUploadError('Ukuran file melebihi 5MB.');
                    setSummary('error', 'Ukuran file terlalu besar. Maksimal 5MB.');
                    setSubmitState();
                    notify('error', 'Ukuran File Terlalu Besar', 'Upload file maksimal 5MB.');
                    return;
                }
            ...
            if (nominalInput) {
                nominalInput.addEventListener('input', () => {
                    applyNominalInputFormatting();
                    clearFieldError(nominalInput);
                    debounceValidateInput(220);
                });
            ...
            [vendorInput, dateInput, kategoriInput, judulInput, deskripsiDetailInput, trxTypeInput].forEach((el) => {
            ...
            kategoriInput?.addEventListener('change', refreshKategoriContext);
            refreshKategoriContext();

            form.addEventListener('submit', (e) => {
                if (nominalInput) {
                    const normalizedNominal = parseNominalNumber(nominalInput.value);
                    nominalInput.value = normalizedNominal > 0 ? String(normalizedNominal) : '';
                }
                ...
                if (submitBtn?.disabled) {
                    ...
                    if (missingFields.length > 0) {
                        shouldShowRequiredErrors = true;
                        renderRequiredFieldErrors(missingFields);
                        setSummary('warning', `Lengkapi field wajib: ${missingFields.map((field) => field.label).join(', ')}.`);
                        notify('warning', 'Field Wajib Belum Lengkap', 'Isi semua field wajib yang ditandai merah.');
                        const firstMissing = missingFields[0]?.el;
                        firstMissing?.focus();
                        firstMissing?.scrollIntoView({ behavior: 'smooth', block: 'center' });
                        return;
                    ...
                    if (!fileInput?.files?.length) {
                        setFileUploadError('Upload bukti transaksi wajib diisi.');
                        setSummary('warning', 'Upload bukti transaksi wajib diisi sebelum pengajuan dikirim.');
                        notify('warning', 'Upload Bukti Wajib', 'Silakan upload dokumen invoice/struk terlebih dahulu.');
                        fileUploadZone?.scrollIntoView({ behavior: 'smooth', block: 'center' });
                        return;
                    }

                    notify('warning', 'Belum Valid', 'Lengkapi data dan tunggu validasi OCR + AI selesai.');
                }
            });
