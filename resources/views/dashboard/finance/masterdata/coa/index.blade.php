...
<div class="dashboard-wrapper">
    <div class="dashboard-container">
        <x-page-header title="Kelola COA" subtitle="Daftar Chart of Accounts yang disinkronkan dari Accurate" :showNotification="true" :showProfile="true" />
        ...
        <div class="stats-grid">
            <div class="stat-card modern">
                <div class="stat-left">
                    <div class="stat-value">{{ \App\Models\COA::count() }}</div>
            ...
            <div class="stat-card modern">
                <div class="stat-left">
                    <div class="stat-value">{{ \App\Models\COA::where('is_active', true)->count() }}</div>
            ...
            <div class="stat-card modern">
                <div class="stat-left">
                    <div class="stat-value">{{ \App\Models\COA::where('is_active', false)->count() }}</div>
        ...
        <section class="modern-section">
            ...
            <div class="filter-container">
                <form id="filterForm" action="{{ route('finance.masterdata.coa.index') }}" method="GET" class="filter-form-finance">
                    <div class="filter-group-pegawai">
                        ...
                        <div class="search-group">
                            ...
                            <input type="text" name="search" id="searchInput" value="{{ request('search') }}" class="filter-input-pegawai search-input" placeholder="Kode, nama COA...">
                        </div>
                    </div>
                    ...
                    <div class="filter-group-pegawai">
                        ...
                        <select name="tipe_akun" id="tipeAkunInput" class="filter-input-pegawai">
                            ...
                            <option value="asset" {{ request('tipe_akun') == 'asset' ? 'selected' : '' }}>Asset</option>
                            <option value="liability" {{ request('tipe_akun') == 'liability' ? 'selected' : '' }}>Liability</option>
                            <option value="equity" {{ request('tipe_akun') == 'equity' ? 'selected' : '' }}>Equity</option>
                            <option value="revenue" {{ request('tipe_akun') == 'revenue' ? 'selected' : '' }}>Revenue</option>
                            <option value="expense" {{ request('tipe_akun') == 'expense' ? 'selected' : '' }}>Expense</option>
                        </select>
                    </div>
                    ...
                    <div class="filter-actions-pegawai">
                        ...
                        <a href="{{ route('finance.masterdata.coa.index') }}" class="btn-reset-pegawai" title="Reset Filter">
                ...
                @else
                    <div class="data-table-wrapper">
                        <table class="data-table">
                            ...
                            <tbody>
                                @foreach ($coas as $coa)
                                    <tr class="{{ $coa->children->count() > 0 ? 'parent-row' : '' }}">
                                        <td data-label="Kode Perkiraan" class="col-kode">
                                            <code class="code-badge">{{ $coa->kode_coa }}</code>
                                        </td>
                                        <td data-label="Nama" class="col-nama">
                                            <div style="font-weight: 700; color: #1e293b; font-size: 0.95rem;">{{ $coa->nama_coa }}</div>
                                        </td>
                                        <td data-label="Deskripsi" class="col-catatan">
                                            <div style="font-size: 0.85rem; color: #64748b; max-width: 150px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;" title="{{ $coa->deskripsi }}">
                                                {{ $coa->deskripsi ?? '-' }}
                                            </div>
                                        </td>
                                        ...
                                        <td data-label="Tipe Akun" class="col-tipe">
                                            <span class="meta-badge" style="text-transform: uppercase; background: rgba(66, 93, 135, 0.08); color: #425d87; padding: 4px 10px; border-radius: 6px; font-size: 0.75rem; font-weight: 600;">{{ $coa->tipe_akun }}</span>
                                        </td>
                                        <td data-label="Sub-Akun" class="col-sub">
                                            <span class="meta-badge" style="background: rgba(66, 93, 135, 0.08); color: #425d87; padding: 4px 10px; border-radius: 6px; font-size: 0.75rem; font-weight: 600;">{{ $coa->children->count() }} sub</span>
                                        </td>
                                        <td data-label="Saldo" class="col-saldo">
                                            <span class="amount-text">{{ $coa->currency_code ?? 'IDR' }} {{ number_format($coa->saldo ?? 0, 0, ',', '.') }}</span>
                                        </td>
                                        <td data-label="Per Tanggal" class="col-as-of">
                                            <span style="font-size: 0.85rem; color: #64748b; font-weight: 500;">
                                                {{ $coa->as_of_date ? $coa->as_of_date->format('d/m/Y') : '-' }}
                                            </span>
                                        </td>
                                        <td data-label="Sinkron" class="col-sync">
                                            <div style="display: flex; align-items: center; justify-content: center; gap: 6px;">
                                                ...
                                                <span style="font-size: 0.75rem; color: #64748b; font-weight: 500;">{{ $coa->last_sync_at ? $coa->last_sync_at->diffForHumans() : '-' }}</span>
                                            </div>
                                        </td>
                                    </tr>
                                    @if ($coa->children->count() > 0)
                                        @foreach ($coa->children as $child)
                                            <tr class="child-row">
                                                <td data-label="Kode Perkiraan" class="col-kode">
                                                    <code class="code-badge" style="margin-left: 1rem; color: #64748b;">{{ $child->kode_coa }}</code>
                                                </td>
                                                <td data-label="Nama" class="col-nama">
                                                ...
                                                <td data-label="Deskripsi" class="col-catatan">
                                                    <div style="font-size: 0.8rem; color: #94a3b8; max-width: 120px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;" title="{{ $child->deskripsi }}">
                                                        {{ $child->deskripsi ?? '-' }}
                                                    </div>
                                                </td>
                                                <td data-label="Status" class="col-status">
                                                ...
                                                <td data-label="Tipe Akun" class="col-tipe">
                                                    <span class="meta-badge" style="text-transform: uppercase; background: rgba(66, 93, 135, 0.05); color: #64748b; padding: 3px 8px; border-radius: 5px; font-size: 0.7rem; font-weight: 600;">{{ $child->tipe_akun }}</span>
                                                </td>
                                                <td data-label="Sub-Akun" class="col-sub">
                                                    <span style="font-size: 0.7rem; color: #94a3b8;">-</span>
                                                </td>
                                                <td data-label="Saldo" class="col-saldo">
                                                    <span class="amount-text" style="font-size: 0.85rem; color: #64748b;">{{ $child->currency_code ?? 'IDR' }} {{ number_format($child->saldo ?? 0, 0, ',', '.') }}</span>
                                                </td>
                                                <td data-label="Per Tanggal" class="col-as-of">
                                                    <span style="font-size: 0.8rem; color: #64748b; font-weight: 500;">
                                                        {{ $child->as_of_date ? $child->as_of_date->format('d/m/Y') : '-' }}
                                                    </span>
                                                </td>
                                                <td data-label="Sinkron" class="col-sync">
                                                    <div style="display: flex; align-items: center; justify-content: center; gap: 6px; color: #cbd5e1;">
                                                        ...
                                                        <span style="font-size: 0.7rem; font-weight: 500;">{{ $child->last_sync_at ? $child->last_sync_at->diffForHumans() : '-' }}</span>
                                                    </div>
                                                </td>
                                            </tr>
                                        @endforeach
                    ...
                    <div class="pagination-wrapper">
                        {{ $coas->links('components.pagination') }}
                    </div>
                @endif
            </div>
        </section>
